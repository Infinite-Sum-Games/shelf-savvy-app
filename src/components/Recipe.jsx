...
import {
  ...
  RECIPE_URL_DELETE,
  RECIPE_URL_EDIT,
} from "../constants"; // Add RECIPE_URL_EDIT in your constants
...
function Recipe() {
  ...
  const [editRecipe, setEditRecipe] = useState({
    ...
    content: "",
    ingredients: [],
  }); // State to hold recipe to be edited
  ...
  const editRecipeHandler = async () => {
    try {
      ...
      const response = await fetch(RECIPE_URL_EDIT, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      // Update the posts state with the edited recipe
      setPosts((prevPosts) =>
        prevPosts.map((post) =>
          post.id === editRecipe.id
            ? {
                ...post,
                title: editRecipe.title,
                content: editRecipe.content,
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-96">
              ...
              >
                ...
                <textarea
                  placeholder="Content"
                  value={editRecipe.content}
                  onChange={(e) =>
                    setEditRecipe({ ...editRecipe, content: e.target.value })
                  }
                  className="w-full mb-4 p-2 border border-gray-300 rounded"
                ></textarea>
                <input
